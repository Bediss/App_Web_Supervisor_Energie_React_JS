...
const SynopticRender = ({ history, prefix = "", synopticObject, autoUpdate = false, name = "" }) => {
        ...
        return data.map((elem, i) => {
            ...
            if (_elem) {
                switch (id) {
                    ...
                    default: {
                        _elem.innerHTML = `${elem.value || 0} ${unitDisplay}`
                    }
                }
            }
            return cc
            ...
            _reportsCodes.map((code)=>{
                ...
                document.querySelectorAll(`#${code}___${cc}___`).forEach((value, i, parent) => {
                    ...
                    value[event]("click", () => {
                        ...
                        history.push({
                            search: `?${params.reportId ? `parent=${params.reportId}&` : ""}&reportId=${code}&counters=${cc}&cLabels=${clElement.Le_Compteur}`,
    ...
    useEffect(() => {
        ...
        // synopticObject.object.MasterObj_Data_Query.cl.find((el)=>el.Code_Compteur=="")
