...
const Modal_CL_V2 = ({ toggle4, modelCl, CL_Tags_Function, Listes_Cl, handleListeCompteurClick, CL_Membre }) => {
    ...
    function Ajouter_Par_Member() {
        ...
        const array = []
        console.log("Object.keys(Member_select)",Object.keys(Member_select).length)
        console.log("CL_Membre_Select",CL_Membre_Select)
        ...
        if (CL_Membre_Select.length == 0) {
            ...
            if (!CL_Membre_Select.find((el) => JSON.stringify(el) == JSON.stringify(Member_select))) {
                ...
                console.log("array2",array.concat(CL_Membre_Select))
            } else {
                Swal.fire({
                    toast: true,
                    position: 'top',
    ...
    useEffect(() => {
        ...
        CL_Tags_Function(CL_Tags_var,showTAGS_CL)
        //console.log("CL_Tags_var", CL_Tags_var)
  
    }, [CL_Tags_var,showTAGS_CL])
